...
use App\Http\Requests\OnSiteAssesment\GetAssessmentRequest;
use App\Http\Requests\OnSiteAssesment\StoreAssessmentAnswersRequest;
use App\Http\Requests\OnSiteAssesment\StoreAssessmentEvidenceRequest;
...
class AssessmentOnSiteController extends Controller
{
    use ApiResponse;
    ...
    public function index(GetAssessmentRequest $request, string $period, int $orgId)
    {
        ...
        $assessment = Assessment::where('organization_id', $orgId)
            ->where('period', $period)
            ->where('type', Assessment::TYPE_OSA)
            ->firstOrFail();
        $domains = [];
        $practiceAreas = [];
        $questions = [];
        ...
        $message = 'On Site Assessment siap diisi.';
        if ($assessment->status == Assessment::STATUS_SUBMITTED) {
            $message = 'On Site Assessment telah diisi.';
        }
    ...
    public function store(StoreAssessmentAnswersRequest $request, string $period, int $orgId)
    {
        ...
        try {
            ...
            return $this->error(
                message: "Status On Site Assessment sudah dikunci dan tidak dapat di-submit.",
                status: 409
            );
        ...
        return $this->success(
            message: 'On Site Assessment berhasil di-submit.',
            status: 200
        );
    }
    ...
    public function storeDraft(StoreAssessmentAnswersRequest $request, string $period, int $orgId)
    {
        ...
        return $this->success(
            message: 'On Site Assessment disimpan sebagai draft.',
            status: 200
        );
    }
    ...
    private function saveAssessmentAnswers(
        ...
        DB::transaction(function () use ($assessment, $storeAsDraft, $updatedAssCriterias, $totalScore) {
            ...
            if ($storeAsDraft && $assessment->status != Assessment::STATUS_DRAFT) {
                $assessment->status = Assessment::STATUS_DRAFT;
            } elseif (!$storeAsDraft && $assessment->status != Assessment::STATUS_SUBMITTED) {
    ...
    public function showExampleAnswersBody(string $period, int $orgId)
    {
        ...
        $assessment = Assessment::where('organization_id', $orgId)
            ->where('period', $period)
            ->whereIn('type', [Assessment::TYPE_OSA, Assessment::TYPE_ODA, Assessment::TYPE_SA])
        ...
        return $this->success(
            message: 'On Site Assessment:example store body generated successfully.',
            data: [
